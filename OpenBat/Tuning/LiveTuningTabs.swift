...
//  discipline from CLAUDE.md, and doubly so here, since these are on screen
...
struct PulseTuningTab: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // The detector has no separate DSP object — these properties ARE
            // the live values, read per column on the main thread. So onLive
            // and onCommit both write here; the split still earns its keep by
            // keeping the `didSet` UserDefaults write to one per gesture… which
            // it can't, since there's only one property. Accepted: these are
            // read at column rate, not per sample, and the write is a scalar.
