...
//  OpenBat
//
//  Per-screen opt-out from the app's flat black navigation bar, for screens
...
//  longer takes effect.** The `.toolbarBackground(Color.black, for:
//  .navigationBar)` calls dotted around this app are inert there; the flat
//  black comes from the UIKit appearance proxy in
//  `OpenBatApp.configureNavigationBarAppearance`, which sets an opaque
//  background on every bar in the app. This is the supported per-screen
//  override of it.
//
...
//  the content up, so on its own it just swaps a black bar for a black gap.
//
