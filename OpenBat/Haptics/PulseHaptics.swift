...
final class PulseHaptics {
    ...
    // checking by hand against the demo clip on a real device. That default
    // briefly read 0.3 and this note warned the actuator was seeing quieter
    // pulses than these numbers assume; it is back at 0.5 (see the property's
    // own note in `PulseDetector`), so the assumption holds again. If the
    // threshold is ever lowered for real, `levelFloor` is the one to revisit
    // first — faint calls will start feeling too strong.
