...
nonisolated enum INatExport {
    ...
    private static func notes(recording: Recording,
        ...
        lines.append("")
        lines.append("**Notes:**")
        lines.append("The sound and every picture here come from the same clip: the bat pass cut out of a longer recording, with a second of air either side. The sound is that clip exactly as recorded — original 384 kHz, real timing, nothing removed, nothing processed — and the numbered spectrograms are consecutive slices of it, so a time on a picture is the same time in the file. It is full spectrum, so it will need a bat sound program or a player that can slow it down before there is anything to hear. Call parameters are produced automatically by OpenBat from the recording and are rounded — they are a quick reference for the eye, not a measurement, and the attached recording carries the full detail if you want to measure it properly. Nothing here has been checked by a person, and the identification suggestion is a best guess by an ML model, please treat all of it as a starting point rather than a result.")
